...
public class SkeletonUtilityInspector : Editor {
	...
	void OnSceneGUI () {
		...
		foreach (Bone b in skeleton.Bones) {
			...
			Vector3 up = transform.TransformDirection(rot * Vector3.right);
			up *= flipRotation;

			SpineEditorUtilities.Icons.boneMaterial.SetPass(0);
			Graphics.DrawMeshNow(SpineEditorUtilities.Icons.boneMesh, Matrix4x4.TRS(vec, Quaternion.LookRotation(transform.up, up), Vector3.one * b.Data.Length * b.WorldScaleX));
		}
	}
