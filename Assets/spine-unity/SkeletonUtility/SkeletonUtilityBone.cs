		...
		if (mode == Mode.Follow) {
			...
			if (scale) {
				...
				if (parentReference == null) {
					...
					if (rotation) {
						float angle = Mathf.LerpAngle(bone.Rotation, Quaternion.LookRotation(flipX ? Vector3.up * -1 : Vector3.up, parentReference.InverseTransformDirection(cachedTransform.up)).eulerAngles.z, overrideAlpha) + flipCompensation;
